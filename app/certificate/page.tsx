        ...
        {certificates.map((cert, i) => (
          <GlowCard key={cert.title + i} delay={(i % 3) * 0.1} className="flex flex-col">
            ...
            >
              ...
              <span
                aria-hidden
                className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#020817] via-[#020817]/40 to-transparent"
              />
              <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-colors duration-300 group-hover:bg-black/45">
                <span className="flex translate-y-1 items-center gap-2 rounded-full bg-sky-500/80 px-3 py-1.5 text-sm font-medium text-white opacity-0 ring-1 ring-white/15 backdrop-blur transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            ...
            <div className="relative flex flex-1 flex-col p-5">
              ...
              <a
                ...
                className="group/link mt-1 inline-flex w-fit items-center gap-1 text-xs text-sky-300 transition-colors hover:text-sky-200 hover:underline hover:underline-offset-2"
              >
                {cert.issuer}
              ...
              {Array.isArray(cert.skills) && cert.skills.length > 0 && (
                <div className="mt-auto flex flex-wrap gap-1.5 border-t border-white/10 pt-3">
                  {cert.skills.map((skill) => (
                    <span
                      key={skill}
                      className="btn-press rounded-full bg-sky-500/10 px-2 py-1 text-[11px] leading-none text-sky-300 ring-1 ring-sky-400/20 hover:bg-sky-500/20 hover:text-white hover:ring-sky-400/50"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="relative border-t border-white/5 p-5 sm:p-7 md:p-8">
                  <motion.div {...lightboxItem(0)}>
                    <span
                      ...
                      className="btn-press group/badge inline-flex h-7 items-center justify-center rounded-full border border-sky-400/25 bg-sky-500/10 px-3 text-sm text-sky-200 hover:border-sky-400/50 hover:bg-sky-500/20 hover:text-white"
                    >
                  ...
                  >
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group/link inline-flex items-center gap-1.5 text-sm text-sky-300 transition-colors hover:text-sky-200 hover:underline hover:underline-offset-2"
                    >
                      {selected.issuer}
                  ...
                  {Array.isArray(selected.skills) && selected.skills.length > 0 && (
                    ...
                    >
                      {selected.skills.map((skill) => (
                        <span
                          key={skill}
                          className="btn-press rounded-full bg-sky-500/10 px-3 py-1.5 text-xs text-sky-200 ring-1 ring-sky-400/25 hover:bg-sky-500/20 hover:text-white hover:ring-sky-400/50"
                        >
                          {skill}
                        </span>
                      ))}
