...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, reason, company, fileKey } = body;

    if (!name || !email || !phone || !reason || !company || !fileKey) {
    ...
    const approveUrl = `${siteUrl}/api/offer-approve?id=${data.id}&token=${actionToken}&action=approve`;
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 500px;">
          <h2>New Offer Letter Access Request</h2>
