...
export async function GET(req: NextRequest) {
  ...
  if (action === "approve") {
    ...
    await resend.emails.send({
      ...
      subject: `Your ${request.company} offer letter request has been approved`,
      html: `
        <div style="font-family: sans-serif; max-width: 500px;">
          <h2>Hi ${request.name},</h2>
          <p>Your request to view the <strong>${request.company}</strong> internship offer letter has been approved.</p>
          <p>You can download it using the button below (link valid for 24 hours):</p>
          <a href="${signedUrlData.signedUrl}" style="background:#8b5cf6;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:12px;">Download Offer Letter</a>
          <p style="margin-top:24px;color:#888;">Thank you for your interest!</p>
        </div>
      `,
      text: `Hi ${request.name},\n\nYour request to view the ${request.company} internship offer letter has been approved.\n\nDownload it here (link valid for 24 hours): ${signedUrlData.signedUrl}\n\nThank you for your interest!`,
    });
